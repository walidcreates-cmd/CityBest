...
function HomePage({ products, onUpdateQty, onCart, onTab, activeTab, onCategoryOpen, categories }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'Hind Siliguri', sans-serif", maxWidth:'100%', margin:'0 auto', paddingBottom:130 }}>
       <div style={{ background:C.green, padding:'10px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100, gap:12 }}>
         <div style={{ flexShrink:0 }}>
           <div style={{ color:'#fff', fontWeight:800, fontSize:18, lineHeight:1 }}>CityBest</div>
           <div style={{ color:'rgba(255,255,255,0.75)', fontSize:9, marginTop:2 }}>시라직걵짤 • 다릅 데리보라리</div>
         </div>
         <div style={{ flex:1, background:'rgba(255,255,255,0.18)', borderRadius:10, display:'flex', alignItems:'center', padding:'8px 12px', gap:8 }}>
           ...
           <input value={search} onChange={e => setSearch(e.target.value)} placeholder="폰독 훴짤운..."
             style={{ border:'none', outline:'none', flex:1, fontSize:13, fontFamily:'inherit', background:'transparent', color:'#fff' }} />
         </div>
         <div style={{ display:'flex', gap:8, flexShrink:0 }}>
           <button onClick={() => onTab('profile')} style={{ background:'rgba(255,255,255,0.15)', border:'none', borderRadius:50, width:36, height:36, fontSize:16, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>??</button>
           <button onClick={onCart} style={{ background:'rgba(255,255,255,0.15)', border:'none', borderRadius:50, width:36, height:36, fontSize:16, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
             ??️
